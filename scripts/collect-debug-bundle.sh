...
if ((CREATE == 0)); then
  printf 'mode=dry-run since=%s sections=manifest,git,image,services,resources,migrations,probes,alerts,logs,smoke,frontend-observability,tempo,versions\n' "$SINCE"
  printf '%s\n' 'files=manifest.json git.txt image.txt services.txt resources.txt migrations.txt probes.txt alerts.json redacted-logs.jsonl smoke-summary.json frontend-observability-summary.json tempo-summary.json versions.txt'
...
printf '{"generatedAt":"%s","since":"%s","sections":["manifest","git","image","services","resources","migrations","probes","alerts","logs","smoke","frontend-observability","tempo","versions"],"requestIdIncluded":%s,"errorIdIncluded":%s,"redaction":"allowlisted structured fields only"}\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$SINCE" "$([[ -n "$REQUEST_ID" ]] && echo true || echo false)" "$([[ -n "$ERROR_ID" ]] && echo true || echo false)" > "$WORK/manifest.json"
