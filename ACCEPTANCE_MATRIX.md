...
| ملخص المراقبة | Admin وغير Admin | مؤشرات Docker-internal فقط | فتح `/observability`، تحقق من إخفاء الرابط والرفض الخادمي لغير Admin، ومحاكاة تعذر مصدر مراقبة | صفحة read-only بلا raw logs/traces/IDs أو أسرار؛ تعذر المراقبة لا يعطل التطبيق | `observability-summary.test.ts`, `work-registry.test.ts`, `/observability` |
| تشخيص الحوادث المحلي | Codex على التطوير | IDs صناعية ومؤشرات Docker bridge فقط | `tahili-diagnose` status/alerts/smoke/request/error/bundle dry-run | مخرجات bounded ومنقحة، IDs صارمة، لا docker exec/raw telemetry/SSRF أو أوامر تغيير | `tahili-diagnose.test.ts`, `scripts/tahili-diagnose.mjs` |
...
| جرد routes | آلي | لا يحتاج بيانات | مقارنة كل `page.tsx` و`route.ts` مع الجرد و`SYSTEM_MAP.md` | 94 صفحة و21 API، غير المفحوص صفر | `route-inventory.test.ts`, `docs/UX_ROUTE_INVENTORY.md` |
