...
// Faro SDK 2.8.2 uses "log" as its normal pushLog level.
const levels = new Set(["debug", "info", "warn", "error", "log"]);
...
function sanitizeRoute(value: unknown) {
  ...
  try {
    const pathname = value.startsWith("/") ? new URL(value, "http://invalid.local").pathname : new URL(value).pathname;
...
export function sanitizeFaroEnvelope(
  input: unknown,
  revision: string,
  ...
  const page = meta?.page as Record<string, unknown> | undefined; const route = sanitizeRoute(page?.url);
  const output: Record<string, unknown> = { meta: { sdk: { name: "faro", version: safeText((meta?.sdk as Record<string, unknown> | undefined)?.version) || "2.8.2" }, app: { name: "tahili-frontend", version: revision, environment: tahiliRuntimeEnvironment(env) }, page: { url: route } } };
