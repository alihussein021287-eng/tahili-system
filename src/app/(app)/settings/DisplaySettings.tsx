...
function Fields({ device, centers, halls }: { device?: Device; centers: Center[]; halls: string[] }) {
  return <>
...
export function DisplaySettings({ devices, centers, halls }: { devices: Device[]; centers: Center[]; halls: string[] }) {
  ...
  return <section id="شاشات الانتظار" className="card p-5">
    ...
    <details className="rounded-xl border border-gray-200 bg-white">
      ...
      <form action={createDisplayDevice} className="grid gap-3 border-t p-4 md:grid-cols-2"><Fields centers={centers} halls={halls} /><div className="md:col-span-2"><button className="btn-primary">إضافة شاشة</button></div></form>
    </details>
    <div className="mt-5 space-y-4">
      {devices.map((device) => <article key={device.id} className="rounded-2xl border border-gray-200 p-4">
        ...
        <details className="rounded-xl border bg-gray-50"><summary className="cursor-pointer px-3 py-2 text-sm font-medium">تعديل الإعدادات</summary><form action={updateDisplayDevice.bind(null, device.id)} className="grid gap-3 border-t p-3 md:grid-cols-2"><Fields device={device} centers={centers} halls={halls} /><div className="md:col-span-2"><button className="btn-ghost" type="submit">حفظ إعدادات الشاشة</button></div></form></details>
