...
const USER_ADMIN_SECTIONS = [
  ...
  { href: "#create-user", label: "إضافة مستخدم" },
  { href: "#filters", label: "البحث والتصفية" },
  { href: "#users-list", label: "قائمة الحسابات" },
];
...
export default async function Users({ searchParams }: { searchParams: Promise<{ q?: string; role?: string; status?: string; branch?: string }> }) {
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      <AdminSectionNav items={USER_ADMIN_SECTIONS} />

      <AdminIntro
        title="لوحة المستخدمين"
        description="إدارة الحسابات مقسمة إلى ملخص سريع، إنشاء حساب جديد، أدوات تصفية، ثم قائمة الحسابات الحالية."
      />
      ...
      <AdminSection
        id="create-user"
        title="إضافة مستخدم"
        description="أنشئ الحساب ببياناته الأساسية وحدد الدور والفرع قبل تسليم كلمة المرور المؤقتة خارج النظام."
      >
      ...
      <AdminSection
        ...
        description="استخدم الحقول التالية لتقليل النتائج قبل الدخول إلى إدارة حساب محدد."
      >
      ...
      <AdminSection
        ...
        description="كل نتيجة تفتح صفحة إدارة مفصلة للبيانات، الأمان، والصلاحيات الخاصة بالمستخدم."
