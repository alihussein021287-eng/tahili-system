...
function PlanFields({ doctors, plan }: any) {
  ...
  return <>
    ...
    <label className="label">عدد أيام العلاج<input name="treatmentDays" type="number" min="1" max="365" defaultValue={plan?.treatmentDays || 30} className="input mt-1" required /></label>
    ...
    <label className="label">قيمة الدورية<input name="reviewInterval" type="number" min="1" max="365" defaultValue={plan?.reviewEveryDays || plan?.reviewEverySessions || 5} className="input mt-1" required /></label>
  </>;
}

export function PatientTherapyProgram({ patientId, referrals, plans, therapists, doctors, halls, canManage, canFinalize }: any) {
  return <div className="min-w-0 space-y-5">
    ...
    <section className="space-y-4"><h2 className="text-lg font-bold">برامج العلاج المرتبطة</h2>{plans.length === 0 ? <p className="text-sm text-gray-500">لا توجد خطة علاج مرتبطة بإحالة مقبولة.</p> : plans.map((plan:any)=><PlanCard key={plan.id} plan={plan} doctors={doctors} canManage={canManage} canFinalize={canFinalize} />)}</section>
  </div>;
}

function PlanCard({ plan, doctors, canManage, canFinalize }: any) {
  ...
  return <article className="min-w-0 rounded-lg border border-gray-200 p-4">
    ...
    {canManage ? <details className="mt-3"><summary className="cursor-pointer text-sm font-medium text-brand-700">تعديل الطبيب والمدة والدورية</summary><form action={updatePhysicalTherapyPlan.bind(null, plan.id)} className="mt-3 grid gap-3 rounded-lg bg-gray-50 p-3 md:grid-cols-2"><PlanFields doctors={doctors} plan={plan} /><div className="md:col-span-2"><button className="btn-primary">حفظ تعديل الخطة</button></div></form></details> : null}
