...
describe("Faro envelope sanitizer", () => {
  ...
  it("uses the closed server environment instead of client metadata", () => {
    const result:any = sanitizeFaroEnvelope({ meta, events:[{name:"stage6b_pipeline_check",domain:"synthetic",attributes:{runId:"safe-run"}}] }, "server-revision", { TAHILI_ENVIRONMENT: "production" });
    expect(result.meta.app).toEqual({name:"tahili-frontend",version:"server-revision",environment:"production"});
    expect(JSON.stringify(result)).not.toContain('"environment":"bad"');
  });
