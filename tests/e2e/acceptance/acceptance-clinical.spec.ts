...
async function diagnosis(browser: Browser, patientId: string, label: string) {
  ...
  await tab(actor.page, "الاستشارية الطبية");
...
async function createReport(browser: Browser, patientId: string, type: "PRELIMINARY" | "FINAL", label: string) {
  const content = `ACCEPTANCE-20260713 ${RUN_ID} تقرير ${label}`; const actor = await pageFor(browser, "DOCTOR"); await actor.page.goto(`/patients/${patientId}`); await tab(actor.page, "الاستشارية الطبية");
  ...
  await submitServerAction({ name: `report-${label}`, role: "DOCTOR", browser, context: actor.context, page: actor.page, submit: form.getByRole("button", { name: "إضافة" }), dbExpectation: async () => Boolean(await prisma.medicalReport.findFirst({ where: { patientId, content } })), confirmation: async p => bodyIncludesOnTab(p, "الاستشارية الطبية", content), recoveryUrl: `/patients/${patientId}?tab=reports`, roleState: statePath(credential("DOCTOR")) });
...
test("التقارير الطبية: أولي ونهائي والجاهزية والطباعة والاعتماد", async ({ browser }) => {
  ...
  const approve = await pageFor(browser, "MANAGER"); await approve.page.goto(`/patients/${patient.id}`); await tab(approve.page, "الاستشارية الطبية"); await submitServerAction({ name: "report-approved", role: "MANAGER", browser, context: approve.context, page: approve.page, submit: approve.page.locator(`tr:has-text("${preliminary.content}")`).getByRole("button", { name: "تأشير مطبوع ومعتمد" }), dbExpectation: async () => (await prisma.medicalReport.findUnique({ where: { id: preliminary.id } }))?.status === "PRINTED_APPROVED", confirmation: async p => bodyIncludesOnTab(p, "الاستشارية الطبية", "مطبوع ومعتمد"), recoveryUrl: `/patients/${patient.id}?tab=reports`, roleState: statePath(credential("MANAGER")) });
...
test("الرقود والأسرّة: التعارض والطباعة والخروج وإعادة التخصيص", async ({ browser }) => {
  ...
  const discharge = await pageFor(browser, "DOCTOR"); await discharge.page.goto(`/patients/${first.id}`); await tab(discharge.page, "الرقود"); const dischargeForm = discharge.page.locator('form:has(button:has-text("تسجيل خروج"))'); await dischargeForm.locator('input[name="dischargeDate"]').fill("2026-07-14"); await submitServerAction({ name: "admission-discharge", role: "DOCTOR", browser, context: discharge.context, page: discharge.page, submit: dischargeForm.getByRole("button", { name: "تسجيل خروج" }), dbExpectation: async () => (await prisma.admission.findUnique({ where: { id: admission.id } }))?.status === "DISCHARGED", confirmation: async p => bodyIncludesOnTab(p, "الرقود", "خرج"), recoveryUrl: `/patients/${first.id}?tab=adm`, roleState: statePath(credential("DOCTOR")) });
